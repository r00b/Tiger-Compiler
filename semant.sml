...
struct
  ...
  fun mismatchErr(typeName, pos, expected, got) = error pos
    ("type mismatch in" ^ typeName ^ ": expected " ^ T.toString(expected) ^ " but got " ^ T.toString(got))
  ...
  fun tyCheckRecordExp(fieldTypes, typ, pos, tenv) =
    let
